    ...
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4 py-8">
      <div className="rounded-[30px] border border-[#EAE1F2] bg-white/80 p-6 shadow-[0_12px_35px_rgba(75,59,90,0.08)]">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-[#4B3B5A]">Create your space.</h1>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <label className="block">
            ...
            />
            {errors.name && <p className="mt-1 text-xs text-[#B05A5A]">{String(errors.name.message)}</p>}
          </label>
        ...
        <p className="mt-5 text-center text-sm text-[#6E5E7A]">
          Already have an account?{' '}
          <Link to="/login" className="font-medium text-[#8B6FB3]">
            Log in
          </Link>
        </p>
      </div>
